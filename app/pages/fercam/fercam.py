...
def fercam():
    ...
    identificativi_sent, identificativi_non_sent = (
        SpedizioniPreliminariService.get_identificativi_partitioned()
    )
    ...
    starting_date_str = (datetime.now() - timedelta(days=int(days_to_fetch))).strftime(
        "%Y%m%d"
    )
    filters = [
    ...
    fatture = mexal.find_warehouse_movements(
        str(datetime.now().year), properties=properties, filters=filters
    )
    ...
    ragioni_sociali = {
        k: (
            v["denominazione"].strip()
            if v["gest_per_fisica"] == "S" and v["denominazione"].strip()
            else v["ragione_sociale"].strip()
        )
    ...
    for f in fatture:
        ...
        f["data_documento"] = datetime.strptime(f["data_documento"], "%Y%m%d").strftime(
            "%d/%m/%Y"
        )
        ...
        f["aspetto"] = (
            mexal_cache.get_aspetto_esteriore(mexal, f["asp_est_beni"]) or "???"
        )
        f["aspetto_icon"] = PACKING_TYPE_ICONS.get(
            PACKING_TYPE_MAP.get(int(f["asp_est_beni"])),
        ...
        f["completo"] = (
            f["aspetto"] != "???"
            and f["nr_colli_sped"] != "0"
            and f["peso_spedizione"] != "0.0"
            and f["ragione_sociale_cliente"] != ""
        )
    ...
    fatture = sorted(
        ...
        key=lambda x: (
            x[
                "data_ult_mod"
            ]  # Parlando con Erika, ha preferito un ordinamento per come le ha create lei su mexal per attaccare le etichette più velocemente
...
def invia():
    ...
    cod_amount_overrides = _safe_json_load(
        request.form.get("cod_amount_overrides", "{}")
    )
    ...
    try:
        ...
        for fattura_id in fatture_ids:
            ...
            nr_colli_override = (
                int(nr_colli_overrides.get(fattura_id))
                if nr_colli_overrides.get(fattura_id)
                else None
            )
        ...
        for _, fatture_batch in groups.items():
            try:
                doc_id, fattura_unica, xml, identificativi = process_fatture_group(
                    mexal, sscc_generator, fatture_batch
                )
                create_spedizione_preliminare(
                    doc_id, fattura_unica, xml, identificativi
                )
                elaborati += 1
...
def preview_invio():
    ...
    if not fatture_ids:
        return (
            jsonify({"message": "Nessun documento selezionato per l'anteprima."}),
            400,
        )
    ...
    try:
        ...
        for fattura_id in fatture_ids:
            ...
            fattura = load_fattura_for_send(
                mexal, sigla, serie, numero, cod_conto, parziale=True
            )
            ...
            preview.append(
                {
                    ...
                    "ragione_sociale_cliente": fattura["indirizzo_spedizione"][
                        "descrizione"
                    ],
                    "is_cod": is_cod,
...
def print_label(ssccs, fattura):
    label_total = (
        int(fattura["nr_colli_sped"][0][1]) if fattura.get("nr_colli_sped") else 1
    )
    ...
    show_personal_zucchero = LABEL_TYPE_MAP.get(
        fattura.get("tipologia_etichetta"), True
    )
...
def get_indirizzo_spedizione(mexal, fattura):
    ...
    if fattura["cod_anag_sped"]:
        ...
        indirizzo_spedizione = mexal.get_indirizzo_di_spedizione(
            fattura["cod_anag_sped"][0][1]
        )
    else:
        ...
        if (
            cliente
            and cliente.get("gest_per_fisica") == "S"
            and cliente.get("denominazione").strip()
        ):
    ...
    if is_paese_it == is_prov_ee:
        raise ValueError(
            f"Contraddizione Paese/Provincia: {indirizzo_spedizione['cod_paese']} / {indirizzo_spedizione['provincia']}"
        )
...
def build_xml(fattura, ssccs):
    ...
    doc_id = generate_doc_id(
        fattura["numero"], fattura["sigla"], int(fattura["data_documento"][:4])
    )
    spedizione = {
        "doc_id": doc_id,
        ...
        "consignee": {
            "name": fattura.get("indirizzo_spedizione", {}).get("descrizione", "")[
                :90
            ],  # 90 char is the limitation for PartnerName in Dachser's system, splitted in 3 lines of 30 char each, so we take the first 90 char to avoid errors
            ...
            "contact": {
                "email": fattura["cliente"].get("email"),
                "phone": fattura["cliente"].get("telefono"),
            },  # 1: note, 2: anagrafica spedizione o cliente
            "type": (
                "AT" if fattura.get("note", {}).get("preavviso") == "S" else None
            ),  # Impostato AT perchè gli altri non ancora attivi (11/06/26)
        },
        "forwarder": {"id": "956"},  # Codice fisso fornito da loro
        "product": Product.TARGOSPEED if fattura["SPEED"] else Product.TARGOFLEX,
        "items": [
            {
                "quantity": fattura["nr_colli_sped"][0][
                    1
                ],  # numeri di cartoni, pallet a perdere o pallet a scambio, a seconda del packing type
                "type": PACKING_TYPE_MAP.get(
                    fattura["asp_est_beni"][0][1]
                ),  # variabile, info presente in fattura
                "description": "Prodotti personalizzati per la ristorazione.",
                "measurements": [
                    {
                        "name": MeasurementName.WEIGHT,
                        "value": fattura["peso_spedizione"][0][1],
                        "unit": UnitCode.KILOGRAM,
                        "code": MeasurementType.GROSS_WEIGHT,
                    },  # presente in fattura
                ],
            },
        ],
...
def load_fattura_for_send(mexal, sigla, serie, numero, cod_conto, parziale=False):
    ...
    fattura = mexal.get_single_warehouse_movement(
        str(datetime.now().year), sigla, serie, numero, cod_conto
    )
    if not fattura:
        raise Exception("Errore nel recupero dei dati della fattura.")
    fattura["riferimento"] = (
        f"{fattura['sigla']} {fattura['serie']}/{fattura['numero']}"
    )

    cliente = mexal_cache.get_customers(mexal, [fattura["cod_conto"]]).get(
        fattura["cod_conto"]
    )
    if not cliente:
    ...
    fattura["cliente"]["telefono"] = (
        contatto_note[:25]
        if contatto_note and str(contatto_note).strip()
        else contatto_cliente
    )

    if str(fattura["id_pagamento"]) in ID_PAGAMENTI_ALLA_CONSEGNA:
        fattura["cod_amount"] = sum(
            Decimal(str(doc[1])) for doc in fattura["tot_doc_pagare"]
        )
    else:
...
def process_fatture_group(mexal, sscc_generator, fatture_info):
    ...
    for (
        ...
        if (
            last_id_pagamento is not None
            and str(f["id_pagamento"]) != last_id_pagamento
        ):
            raise ValueError(
                "Non è possibile raggruppare fatture con modalità di pagamento diverse."
            )
        last_id_pagamento = str(f["id_pagamento"])

        # Override individuali
        f["SPEED"] = is_speed
        if telefono_override is not None:
            f["cliente"]["telefono"] = telefono_override
        if nr_colli_override is not None:
            f["nr_colli_sped"][0][1] = nr_colli_override
        if peso_override is not None:
            f["peso_spedizione"][0][1] = peso_override
        if (
            cod_amount_override is not None
            and str(f.get("id_pagamento")) in ID_PAGAMENTI_ALLA_CONSEGNA
        ):
            f["cod_amount"] = cod_amount_override
    ...
    for f in fatture[1:]:
        ...
        if f.get("cod_amount") is not None:
            merged["cod_amount"] = (merged.get("cod_amount") or Decimal(0)) + f[
                "cod_amount"
            ]
    ...
    if (
        "SOSTA TECNICA"
        in merged.get("indirizzo_spedizione", {}).get("descrizione", "").upper()
    ):
        merged["indirizzo_spedizione"]["descrizione"] = merged.get("note", {}).get(
            "sosta_tecnica_ragione_sociale"
        )
        merged["indirizzo_spedizione"]["indirizzo"] = merged.get("note", {}).get(
            "sosta_tecnica_indirizzo"
        )
        merged["indirizzo_spedizione"]["localita"] = merged.get("note", {}).get(
            "sosta_tecnica_localita"
        )
        merged["indirizzo_spedizione"]["cap"] = merged.get("note", {}).get(
            "sosta_tecnica_cap"
        )
        merged["indirizzo_spedizione"]["cod_paese"] = merged.get("note", {}).get(
            "sosta_tecnica_cod_paese"
        )
