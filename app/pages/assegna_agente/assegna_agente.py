...
def get_contact():
    ...
    return jsonify(contact=contact, company=company)
...
def assegnaAgente():
    ...
    if not hubspot:
        ...
        return render_template(
            "assegna-agente.html", agents=[], contact_source_options=[]
        )

    agents_by_id = get_active_agents_by_id(hubspot)
    contact_source_options = hubspot.getContactPropertyInfo("fonte").get("options", [])
    contact_source_options_by_value = {
        opt["value"]: opt["label"] for opt in contact_source_options
    }
    ...
    updated_contact, updated_company = upsert_contact_and_company(
        hubspot, contact_info, company_info
    )
    ...
    updated_contact["fonte"] = contact_source_options_by_value.get(
        updated_contact.get("fonte"), updated_contact.get("fonte")
    )
    ...
    if mailer:
        ...
        flash(
            "Attenzione: Email non inviate (configurazione mailer mancante).", "warning"
        )
...
def get_active_agents_by_id(hubspot):
    ...
    agents = hubspot.getContactBatch(
        agents_ids,
        [
            "firstname",
            "lastname",
            ...
            "escluso_da_assegnazione_clienti",
        ],
    )
    filtered_agents = [
        a
    ...
    sorted_agents = sorted(
        filtered_agents,
        key=lambda a: (
            (a.get("lastname") or "zzzzzz").lower(),
            (a.get("firstname") or "").lower(),
        ),
    )
    ...
    associated_contacts_ids = [
        item for sublist in associations.values() for item in sublist
    ]
    associated_contacts = hubspot.getContactBatch(
        list(associated_contacts_ids), ["firstname", "lastname", "email"]
    )
        ...
        if agent_id in agents_by_id:  # Controllo non sia un agente escluso
            for contact_id in contacts_ids:
                for assignment in firebase_assignments:
                    if (
                        assignment["cliente"] == contact_id
                        and assignment["agente"] == agent_id
                    ):
                        associated_contacts_by_id[contact_id]["operatore"] = (
                            assignment.get("operatore")
                        )
...
def upsert_contact_and_company(hubspot, form_contact, form_company):
    ...
    hubspot_contact, hubspot_company = get_contact_and_its_company(
        hubspot, form_contact["email"]
    )
    ...
    if contact_id and form_company != {}:
        if hubspot_company:
            ...
            if company_id:
                hubspot.createContactsAssociatedCompaniesBatch(
...
def create_deal_for_agent(hubspot, agent, contact, company):
    ...
    hubspot.createContactsAssociatedDealsBatch(
        [(deal_id, contact["id"]), (deal_id, agent["id"])]
    )
