...
def generate_lotto_label(lotto, data, ddt, fornitore, tipologia_zucchero, lotti, note):
    zpl_rows = [
        "^XA",
        "^CI28",
        "^PW839",
        ...
        "^LH0,0",
        "^FWR",
        "^FO710,399^GB120,400,120^FS",
        f"^FO700,50^CF0,100^FR^FB1098,1,0,C^FD{lotto}\\&^FS",
        f"^FO740,820^CF0,45^FB330,1,0,R^FD{data}^FS",
        f"^FO740,60^CF0,45^FB330,1,0,L^FDDDT: {ddt}^FS",
        f"^FO530,60^CF0,65^FB600,2,10,L,0^FD{fornitore}^FS",
        f"^FO450,60^CF0,43^FD{tipologia_zucchero}^FS",
        "^FO350,60^CF0,40^FDLOTTI FORNITORI^FS",
        f"^FO230,60^CF0,35^FB600,3,0,L^FD{', '.join(lotti)}^FS",
        f"^FO210,650^BQN,2,100^FDHA,https://personal-zucchero.vercel.app/lotto/scan/?id={lotto}^FS",
        "^FO200,50^GB3,1100,3^FS",
        f"^FO25,50^CF0,65^FB1098,2,0,C^FD{note}\\&^FS",
        "^XZ",
        ]
    return "\n".join(zpl_rows)
