...
def create_personal_zucchero_consignor():
    ...
    contact = Contact(
        name="Personal Zucchero", phone="0541629284", email="info@personalzucchero.it"
    )

    return Consignor(
        id="72859708", name="PERSONAL ZUCCHERO S.R.L.", address=address, contact=contact
    )
...
def create_xml(nuova_spedizione):
    try:
        ...
        consignee = Consignee(
            ...
            contact_type=(
                DachserContactType(nuova_spedizione["consignee"]["type"])
                if nuova_spedizione["consignee"]["type"] is not None
                else None
            ),
        )

        forwarder = Forwarder(id=nuova_spedizione["forwarder"]["id"])

        cod_code = nuova_spedizione.get("cod_code")
        cod_amount = nuova_spedizione.get("cod_amount")
        cod = (
            CodDetails(code=cod_code, amount=cod_amount, currency=Currency.EUR)
            if cod_amount is not None and cod_code is not None
            else None
        )
