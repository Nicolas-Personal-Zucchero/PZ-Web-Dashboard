...
class ReviewService:
    ...
    def does_review_exist(email: str) -> bool:
        return (
            ...
            .filter_by(customer_email=email, hidden=False)
            .first()
            is not None
        )
    ...
    @staticmethod
    def create(
        customer_name: str, customer_email: str, language: str, sender_id: int
    ) -> Review:
        try:
            review = Review(
            ...
            db.session.rollback()
            raise ValueError(
                "Errore di integrità: dipendente inesistente o vincolo violato"
            ) from e
